...
2. BFS от позиции атакующего до позиции цели
3. 8 направлений движения (включая диагонали)
4. Восстановление пути через массив родителей
5. Если путь не найден — возврат пустого списка

**Константы:**
- WIDTH = 27 (ширина поля)
- HEIGHT = 21 (высота поля)

**Алгоритмическая сложность: O(W × H)**
...
- Итого: O(W × H) = O(27 × 21) = O(567) — фактически константа для данного поля

Примечание: Требуемая сложность O(W × H × log(W × H)) достигается при использовании алгоритма Дейкстры или A*. BFS обеспечивает даже лучшую сложность O(W × H), так как все рёбра имеют одинаковый вес.
